...
const char *srcImg = "vertical.jpg";
...
void CalcMomentos(const Mat &sourceImage)
{
    float area = 0;
    float m10 = 0;
    float m01 = 0;
    ...
    for(int y=0; y<sourceImage.rows;++y)
        for(int x=0; x<sourceImage.cols; ++x){
            if(sourceImage.at<Vec3b>(Point(x,y))[1] < binTreshold){
                area++; //m00
                m10 = m10 + x;
                m01 = m01 + y;
            }
    ...
    x_average = m10 / area;
    y_average = m01 / area;
    cout << "  Area:" << area <<"  Xcent:" << x_average <<"  Ycent:" << y_average;
    cout << endl;

    for(int y=0; y<sourceImage.rows;++y)
        for(int x=0; x<sourceImage.cols; ++x){
            if(sourceImage.at<Vec3b>(Point(x,y))[1] < binTreshold){
                u02 = pow(y - y_average, 2) + u02;
                u20 = pow(x - x_average, 2) + u20;
                u11 =((y - y_average)*(x - x_average)) + u11;
            }
        }

    theta = (180/3.14159)*0.5 * atan2((2*u11),(u20-u02));
    ...
    h1 = n20 + n02 ;
    h2 = (pow(n20 - n02, 2) + 4 * pow(n11, 2));

    cout <<"  u02:" << m10 <<"  h1:" << h1 <<"  h2:" << h2  <<"  theta:" << theta;
    cout << endl;
